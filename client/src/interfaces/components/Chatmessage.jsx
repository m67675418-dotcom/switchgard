...
/* ══════════════════════════════════════
   CONTACTS MODAL
══════════════════════════════════════ */
function ContactsModal({ cfg, onClose, onCreate }) {
  ...
  useEffect(() => {
    Promise.all(
      Object.entries(ROLE_NAME_KEY).map(([role, nameKey]) =>
        fetch(`${API}/${role}/getAll`)
          .then(r => r.json())
          .then(data => (Array.isArray(data) ? data : []).map(p => ({
            id:     p._id,
            name:   p[nameKey] || p.fullName || p.userId || p.matricule || p.nomPharmacie || "?",
            role,
    ...
    ).then(all => { setContacts(all.flat()); setLoading(false); });
  }, []);
