...
import "./DoctorMessage.css"; // Changé de .module.css à .css

export default function DoctorMessage({ onNavigate }) {
  ...
  const handleSend = async () => {
    if (!newMsg.sender || !newMsg.receiver || !newMsg.content) {
  ...
  const handleDelete = async (id) => {
    try {
      await fetch(`http://localhost:5000/deletemessage/${id}`, { method: "DELETE" });
    ...
    <div className="container">
      ...
      {showForm && (
        <div className="msgForm">
          ...
          {[
            { key: "sender", label: "From", placeholder: "Sender name" },
            { key: "receiver", label: "To", placeholder: "Receiver name" },
            ...
            <div key={key} className="formGroup">
              ...
              <input
                ...
                value={newMsg[key]}
                onChange={(e) => setNewMsg({ ...newMsg, [key]: e.target.value })}
              />
            </div>
          ))}
